...
class MetricWidget extends AsyncView<Props, State> {
  ...
  handleAddQuery = () => {
    this.setState(state => {
      ...
      newState.queries.push({
        ...newQuery,
        aggregation: state.metric?.operations[0] ?? '',
      });
      return newState;
    });
  };
  ...
  handleProjectChange = (selectedProjects: number[]) => {
    const {projects, router, location, organization, params} = this.props;
    ...
    if (newlySelectedProject?.id) {
      router.replace({
        pathname: `/organizations/${organization.slug}/dashboards/${dashboardId}/widget/new/`,
        query: {
          ...location.query,
...
const Wrapper = styled('div')`
  margin: -${space(2)} -${space(4)} -${space(3)};
